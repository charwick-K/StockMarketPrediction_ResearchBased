# =============================================================================
# Structured Temporal Regressor
# LSTM for Memory Extraction + ANN for Decision Modeling
#
# Key Idea:
#   Temporal dynamics and decision boundaries are different problems.
#   This architecture explicitly separates them.
# =============================================================================
...
class TemporalFeatureDataset(Dataset):
    """
    Produces (sequence ? latent memory ? price-aware evaluation)
    """
...
# MODEL
# =============================================================================
class StructuredTemporalRegressor(nn.Module):
    """
    Stage 1: LSTM extracts temporal memory
    Stage 2: ANN maps memory ? prediction
    """
    ...
    def forward(self, x):
        ...
        latent_state = memory[:, -1, :]
        return self.regressor(latent_state).squeeze(-1)
...
print("\n===== Structured Temporal Regressor | CV =====\n")

for fold, (train_idx, val_idx) in enumerate(tscv.split(df_trainval), start=1):
    print(f"--- Fold {fold} ---")

    train_ds = TemporalFeatureDataset(df_trainval.iloc[train_idx], WINDOW_SIZE)
    val_ds = TemporalFeatureDataset(df_trainval.iloc[val_idx], WINDOW_SIZE)

    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=False)
    val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False)

    model = StructuredTemporalRegressor().to(DEVICE)
...
final_loader = DataLoader(
    TemporalFeatureDataset(df_trainval, WINDOW_SIZE),
    batch_size=BATCH_SIZE,
    shuffle=False,
)

final_model = StructuredTemporalRegressor().to(DEVICE)
...
test_loader = DataLoader(
    TemporalFeatureDataset(df_test, WINDOW_SIZE),
    batch_size=BATCH_SIZE,
    shuffle=False,
)
